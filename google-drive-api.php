...
function Drive($link) {
	...
	if(file_exists('cache/'.$file_name.'.txt')) {
		$fopen = file_get_contents('cache/'.$file_name.'.txt');
		$data = explode('@@', $fopen);
...
function gd_cache($link, $source) {
	...
	$file = fopen("cache/".$file_name.".txt",'w');
	fwrite($file,$string);
	fclose($file);
...
function getlink($id){
	...
	curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/video.mp4");
	curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . "/video.mp4");
